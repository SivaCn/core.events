...
class SimpleServer(SimpleRabbitMQ):
    def __init__(self, **mq_details):
...
def event_emittor():
    for i in range(5):
        emitter = SimpleProducer(host='localhost', port=5672)
        emitter.emit('hello', 'PowerUp')

def event_listener():
    listener = SimpleServer(host='localhost', port=5672)
